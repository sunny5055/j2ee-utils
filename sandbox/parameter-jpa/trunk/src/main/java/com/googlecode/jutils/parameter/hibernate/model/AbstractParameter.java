...
@NamedQueries({ @NamedQuery(name = ParameterDao.COUNT_BY_NAME, query = "select count(*) from AbstractParameter p where p.name = :name"),
		@NamedQuery(name = ParameterDao.FIND_BY_NAME, query = "from AbstractParameter p where p.name = :name") })
@SuppressWarnings("serial")
public abstract class AbstractParameter<V extends Serializable> extends AbstractDto<Integer> {
	@Id
