...
public class ${className} {
	...
	public static void main(String[] args) throws IOException {
		context = new ClassPathXmlApplicationContext("${util.getClassPathResource(config.springBusinessFilePath, config.springBusinessFileName)}");
	...
	private static void writeScript(String file, String[] lines) throws IOException {
		...
		if (entityManagerFactory != null) {
			...
			if (!MapUtil.isEmpty(jpaPropertyMap)) {
				for (final Map.Entry<String, Object> entry : jpaPropertyMap.entrySet()) {
					configuration.setProperty(entry.getKey(), entry.getValue().toString());
				}
			}
